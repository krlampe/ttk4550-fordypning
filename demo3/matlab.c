...
/* Returns a pointer to the name on the left-hand side of the AST */
static char *get_LHS(ast_node *root);
...
void matlab_generate(FILE *out) {
  ...
  char *var_names[nr_of_equations];

  /* Heading */
  fputs("%% Simulator generated by sim-gen.\n% ODE:\n", out);
  for (int i = 0; i < nr_of_equations; i++) {
    ast_node *root = symbol_table[i].ast_root;
    var_names[i] = get_LHS(root);
  ...
  */
  ...
  fprintf(out, "legend('%s'", var_names[0]);
  for (int i = 1; i < nr_of_equations; i++) {
